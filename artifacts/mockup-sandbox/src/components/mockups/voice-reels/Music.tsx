...
const MUSIC_TRACKS = [
  { id: 'm1', title: 'Bollywood Beats', artist: 'DJ Ridhi', duration: 180, cover: '🎵' },
  { id: 'm2', title: 'Lo-Fi India', artist: 'Chill Studio', duration: 240, cover: '🎹' },
  { id: 'm3', title: 'Desi Trap', artist: 'Trap King', duration: 200, cover: '🎺' },
  { id: 'm4', title: 'Classical Mix', artist: 'Sitar Vibes', duration: 300, cover: '🎸' },
  { id: 'm5', title: 'Folk Fusion', artist: 'Village Beats', duration: 160, cover: '🎺' },
];

const VOICE_REELS = [
  { id: "v1", userName: "Rohan Joshi", userCity: "Mumbai", caption: "Why every Indian family has a WhatsApp group called 'Family' 😂", likes: 18200, comments: 1204, shares: 340, isLiked: false, duration: 42, category: "Comedy", plays: 218400, themeId: 'sunset', musicTrack: 'm1' },
  { id: "v2", userName: "Priya Kapoor", userCity: "Delhi", caption: "Startup India: how to raise your first crore 🚀", likes: 9400, comments: 534, shares: 210, isLiked: true, duration: 18, category: "Business", plays: 154000, themeId: 'aurora', musicTrack: 'm2' },
  { id: "v3", userName: "Amit Verma", userCity: "Bangalore", caption: "Cricket World Cup predictions that actually make sense 🏏", likes: 25600, comments: 892, shares: 560, isLiked: false, duration: 36, category: "Cricket", plays: 312000, themeId: 'ocean', musicTrack: null },
];
...
function MusicOverlay({ track, visible, onClose, onSelect }: { track: typeof MUSIC_TRACKS[0] | null, visible: boolean, onClose: () => void, onSelect: (id: string) => void }) {
    ...
    <div className="absolute inset-0 z-30 bg-black/60 backdrop-blur-sm flex items-end">
      <div className="w-full bg-[#1a1a2e] rounded-t-3xl border-t border-white/10 p-4 pb-8 animate-slide-up">
        ...
        <div className="flex items-center justify-between mb-4">
          <h3 className="text-[16px] font-bold text-white">Background Music</h3>
        ...
        <div className="space-y-2">
          {MUSIC_TRACKS.map(t => (
            <button key={t.id} onClick={() => onSelect(t.id)} className="flex items-center gap-3 w-full p-3 rounded-xl bg-white/5 hover:bg-white/10 transition-colors">
              <span className="text-2xl">{t.cover}</span>
              <div className="flex-1 text-left">
                <p className="text-[14px] font-semibold text-white">{t.title}</p>
                <p className="text-[12px] text-white/60">{t.artist} · {formatDuration(t.duration)}</p>
              </div>
              {track?.id === t.id && (
                <div className="flex gap-[2px] items-end h-4">
                  {[0.3,0.6,0.4,0.8].map((h,i) => (
                    <div key={i} className="w-1 bg-[#E91E8C] rounded-full animate-bounce" style={{ height: `${h*16}px`, animationDelay: `${i*0.1}s` }} />
                  ))}
                </div>
              )}
            </button>
        ...
        <button className="w-full mt-4 py-3 rounded-full bg-gradient-to-r from-[#7B2FBE] to-[#E91E8C] text-white font-semibold text-[14px]">
          Browse Full Library
        </button>
      </div>
    </div>
...
function ReelCard({ reel, active }: { reel: typeof VOICE_REELS[0], active: boolean }) {
  ...
  const [currentTrack, setCurrentTrack] = useState<typeof MUSIC_TRACKS[0] | null>(MUSIC_TRACKS.find(t => t.id === reel.musicTrack) || null);
  ...
  return (
    <div className="relative w-full h-full overflow-hidden flex-shrink-0">
      <div className="absolute inset-0 bg-gradient-to-br from-[#1a0b2e] via-[#4a1a6b] to-[#00d4aa]" />
      ...
      {currentTrack && (
        <div className="absolute top-28 left-4 right-4 z-20">
          <div className="bg-black/40 backdrop-blur-md rounded-2xl border border-white/10 p-3 flex items-center gap-3">
            <span className="text-xl">{currentTrack.cover}</span>
            <div className="flex-1 min-w-0">
              <p className="text-[12px] font-semibold text-white truncate">{currentTrack.title}</p>
              <p className="text-[10px] text-white/60">{currentTrack.artist}</p>
            </div>
            <div className="flex gap-[2px] items-end h-3">
              {[0.3,0.6,0.4,0.8].map((h,i) => (
                <div key={i} className="w-[3px] bg-[#E91E8C] rounded-full" style={{ height: playing ? `${h*12}px` : '3px', transition: 'height 0.3s' }} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="absolute top-28 right-4 z-20">
        <button onClick={() => setShowMusic(true)} className="w-10 h-10 rounded-full bg-black/40 border border-white/20 flex items-center justify-center backdrop-blur-sm text-lg">
          🎵
        </button>
      </div>
      ...
      <MusicOverlay track={currentTrack} visible={showMusic} onClose={() => setShowMusic(false)} onSelect={(id) => { setCurrentTrack(MUSIC_TRACKS.find(t => t.id === id) || null); setShowMusic(false); }} />
    </div>
  );
}
